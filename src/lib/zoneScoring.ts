...
export interface ZoneScoreData {
  ...
  export class ZoneScoringSystem {
    ...
    calculateZoneScore(): ZoneScoreData {
      console.log('Calculating zone score, history length:', this.analysisHistory.length);
      ...
      if (this.scoreHistory.length > 1) {
        ...
        const recentScores = this.scoreHistory.slice(-Math.min(5, this.scoreHistory.length));
        smoothedScore = recentScores.reduce((sum, score) => sum + score, 0) / recentScores.length;
      }
      
      // More responsive smoothing: blend with previous smoothed score
      if (this.scoreHistory.length > 3) {
        const previousSmoothed = this.scoreHistory.slice(-2).reduce((sum, score) => sum + score, 0) / 2;
    ...
    private calculateZoneBreakdownFromFrames(frames: typeof this.analysisHistory): ZoneBreakdown {
      ...
      for (let i = 0; i < frames.length; i++) {
        ...
        if (frame.userConfidence < 0.1 || frame.userPitch <= 0) {
          console.log(`Skipping frame ${i}: confidence=${frame.userConfidence}, pitch=${frame.userPitch}`);
          continue;
        }
        ...
        if (frame.targetPitch <= 0) {
          ...
          if (frame.volume > 0.3 && frame.userPitch > 0) {
            ...
            console.log(`Frame ${i}: No target pitch, good volume/pitch - keepTrying`);
            ...
            console.log(`Frame ${i}: No target pitch, very low volume - completelyOff`);
          }
        } else {
    ...
    private calculateZoneBreakdown(): ZoneBreakdown {
      ...
      for (let i = 0; i < this.analysisHistory.length; i++) {
        ...
        if (frame.userConfidence < 0.1) {
          console.log(`Skipping frame ${i} due to low user confidence:`, frame.userConfidence);
          continue;
        }
    ...
    getScoreFeedback(score: ZoneScoreData): string {
      const { perfectZone, keepTryingZone, farOffZone, completelyOffZone, silenceReward, totalScore } = score;
    ...
    getZoneFeedback(score: ZoneScoreData): string[] {
      ...
      const { perfectZone, keepTryingZone, farOffZone, completelyOffZone, silenceReward } = score;
