...
export class AudioAnalyzer {
  ...
  calculateScoreFromNotes(userNote: string, targetNote: string, confidence: number = 1.0): KaraokeScore {
    ...
    let averageScore = confidenceWeightedScore;
    if (this.scoreHistory.length > 1) {
      // Use weighted average with more weight on recent scores
      const recentScores = this.scoreHistory.slice(-Math.min(10, this.scoreHistory.length));
      const weights = recentScores.map((_, index) => (index + 1) / recentScores.length);
      const weightedSum = recentScores.reduce((sum, score, index) => sum + (score * weights[index]), 0);
      const weightSum = weights.reduce((sum, weight) => sum + weight, 0);
      averageScore = weightedSum / weightSum;
    }
  ...
  private noteDifferenceToScore(noteDifference: number): number {
    // More accurate scoring based on note differences
    if (noteDifference <= 0.1) {
      return 100; // Perfect - exact match
    } else if (noteDifference <= 0.5) {
      return 98; // Near perfect - very close
    } else if (noteDifference <= 1) {
      return 95; // Excellent - same note or very close
    } else if (noteDifference <= 1.5) {
      return 90; // Very good - adjacent note
  ...
  addAnalysisFrameToZoneScoring(
    ...
    // Add frames more liberally to capture all audio activity
    if (userPitch > 0 || volume > 0.05) {
